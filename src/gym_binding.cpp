...
static std::size_t curl_save_to_string(void* buffer,
                                       std::size_t size,
                                       std::size_t nmemb,
                                       void* userp) {
 ...
 public:
  ...
  void throw_server_error_or_response_code(const std::string& answer, Json::Value& j) {
    ...
    if (response_code != 200 && j.isObject() && j.isMember("message")) {
      ...
      throw std::runtime_error("bad HTTP response code, and also cannot parse server message: " +
                               answer);
    } else {
 ...
 public:
  ...
  void step(const std::vector<float>& action, bool render, State* save_state_here) override {
    ...
    if (aspace->type == Space::DISCRETE) {
      ...
      assert(action.size() ==
             aspace->box_low.size());  // really assert, it's a programming error on C++ part
